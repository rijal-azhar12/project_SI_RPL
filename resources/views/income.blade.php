...
<div class="container">
    ...
    <div class="stat-cards-container">
        ...
        <div class="stat-card">
            ...
            <span class="stat-subtitle">Across all transactions</span>
        </div>
        <div class="stat-card">
    ...
    <div class="filter-bar">
        ...
        <div class="filter-controls">
            <a href="{{ route('income.index', ['filter' => 'Day']) }}" class="filter-btn {{ $filter == 'Day' ? 'active' : '' }}">Day</a>
            <a href="{{ route('income.index', ['filter' => 'Week']) }}" class="filter-btn {{ $filter == 'Week' ? 'active' : '' }}">Week</a>
            <a href="{{ route('income.index', ['filter' => 'Month']) }}" class="filter-btn {{ $filter == 'Month' ? 'active' : '' }}">Month</a>
        </div>
    </div>
    ...
    <div class="table">
        ...
        <div class="table-row">
            <div class="item-number">#{{ $transaksi->id_transaksi }}</div>
            <div class="item-name">{{ $transaksi->user->nama ?? 'N/A' }}</div>
            <div class="item-description">{{ $transaksi->tanggal_transaksi }}</div>
            <div class="item-units">{{ $transaksi->details->sum('jumlah_item') }}</div>
            <div class="item-price">Rp{{ number_format($transaksi->details->sum('subtotal'), 0, ',', '.') }}</div>
            <div class="item-actions">
                <div class="action-btn edit-btn" data-id="{{ $transaksi->id_transaksi }}" data-details="{{ json_encode($transaksi) }}">
        ...
        @empty
        <div class="table-row">
            <div class="item-number" style="grid-column: 1 / -1; text-align: center;">Tidak ada data pemasukan ditemukan.</div>
        </div>
        @endforelse
    </div>
...
<div class="modal" id="incomeModal">
    <div class="modal-content large">
        ...
        <form id="incomeForm" method="POST" action="">
            ...
            <button type="button" class="btn btn-secondary" id="addMenuItemBtn" style="margin-bottom: 20px;">+ Tambah Item</button>
...
<template id="menu_item_template">
    <div class="form-row menu-item-row">
        ...
        <div class="form-group">
            <label>Jumlah *</label>
            <input type="number" name="menu_items[][jumlah_item]" class="form-control menu_item_jumlah" min="1" value="1" required>
        </div>
        <div class="form-group">
        ...
        function populateForm(transaksi) {
            ...
            if (transaksi.details && Array.isArray(transaksi.details)) {
                transaksi.details.forEach(detail => {
                    ...
                    const selectedMenu = newRow.querySelector('.menu_item_id_menu').options[newRow.querySelector('.menu_item_id_menu').selectedIndex];
                    newRow.querySelector('.menu_item_harga').value = selectedMenu.dataset.harga || detail.subtotal / detail.jumlah_item;
                });
            }
        }
